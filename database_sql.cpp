...
void Database_sql::invoices_createNewData(
    QString supplierId, QString carId, QString kol, QString sale, QString orderId)
{
    ...
    std::thread th([&]() {
        ...
        selectQuery.exec(
            "INSERT INTO Invoice (id_supplier, id_car, kolvo, sale, id_order) VALUES ('"
            + supplierId + "', '" + carId + "', '" + kol + "', '" + sale + "', '" + orderId + "')");
    });
    qDebug() << "invoice";
    th.join();
}

void Database_sql::invoices_addNewData(
    QString supplierId, QString carId, QString kol, QString sale, QString invoiceId)
{
    //std::thread th([&](){
    QSqlQuery selectQuery(db);
    selectQuery.exec("UPDATE Invoice SET id_supplier = '" + supplierId + "', id_car = '" + carId
                     + "', kolvo = '" + kol + "', sale = '" + sale + "' WHERE id_invoice = '"
                     + invoiceId + "'");
...
QVector<QString> Database_sql::getInvoiceDataVector(QString name, QString sort, int order)
{
    ...
    std::thread th([&]() {
        ...
        selectQuery.exec("SELECT * FROM " + name + " WHERE id_order = '" + QString::number(order)
                         + "' ORDER BY " + sort + " ASC");
...
QString Database_sql::getTextSupplier(QString vecData)
{
    ...
    std::thread th([&]() {
        QSqlQuery selectQuery(db);
        selectQuery.exec("SELECT * FROM Supplier WHERE id_supplier = '" + vecData
...
QVector<QString> Database_sql::getSupplierVectorName(int index)
{
    ...
    std::thread th([&]() {
        ...
        QSqlQuery selectQuery(db);
        selectQuery.exec("SELECT Company FROM Invoice INNER JOIN Supplier ON Invoice.id_supplier = "
                         "Supplier.id_supplier WHERE Invoice.id_order = '"
                         + QString::number(index) + "'ORDER BY id_invoice asc");
...
QVector<QString> Database_sql::getCarVectorName(int index)
{
    ...
    std::thread th([&]() {
        //qDebug() << "getDataVector thread: " << QThread::currentThreadId();
        QSqlQuery selectQuery(db);
        selectQuery.exec("SELECT Model FROM Cars INNER JOIN Invoice ON Cars.id_car = "
                         "Invoice.id_car WHERE Invoice.id_order = '"
                         + QString::number(index) + "'ORDER BY id_invoice asc");
...
int Database_sql::getIndexOrder(int vecData, bool curs)
{
    ...
    std::thread th([&]() {
        QSqlQuery selectQuery(db);
        if (curs)
            selectQuery.exec("SELECT MIN(id_order) FROM Orders WHERE id_order > "
                             + QString::number(vecData) + "");
        else
            selectQuery.exec("SELECT MAX(id_order) FROM Orders WHERE id_order < "
                             + QString::number(vecData) + "");
        while (selectQuery.next())
...
int Database_sql::getIndexClients(QVector<QString> vecName, QString vecData)
{
    ...
    std::thread th([&]() {
        QSqlQuery selectQuery(db);
        selectQuery.exec("SELECT * FROM Clients WHERE id_client = '" + vecData
...
int Database_sql::getIdClientName(QString company)
{
    ...
    std::thread th([&]() {
        QSqlQuery selectQuery(db);
        selectQuery.exec("SELECT * FROM Clients WHERE Company = '" + company
                         + "' ORDER BY id_client ASC");
...
QString Database_sql::getClientName(QString id)
{
    ...
    std::thread th([&]() {
        QSqlQuery selectQuery(db);
        selectQuery.exec("SELECT * FROM Clients WHERE id_client = '" + id
                         + "' ORDER BY id_client ASC");
...
void Database_sql::orders_createNewData(QVector<QString> vec)
{
    //std::thread th([&](){
    QSqlQuery selectQuery(db);
    selectQuery.exec("INSERT INTO Orders (id_client, id, execution_date, placement_date, "
                     "assignment_date, delivery_address, sumcost_new) VALUES ('"
...
void Database_sql::orders_addNewData(QVector<QString> vector)
{
    std::thread th([&]() {
        QSqlQuery selectQuery(db);
        selectQuery.exec("UPDATE Orders SET id_client = '" + vector[1] + "', id = '" + vector[2]
                         + "', execution_date = '" + vector[3] + "', placement_date = '" + vector[4]
                         + "',  assignment_date = '" + vector[5] + "',  delivery_address = '"
                         + vector[6] + "',  sumcost_new = '" + vector[7] + "'  WHERE id_order = '"
                         + vector[0] + "';");
        //selectQuery.exec("UPDATE Orders SET id_client = '" + vector.at(1) +"', id = '" + vector.at(2)+"', delivery_address = '" + vector.at(6) +"',placement_date = '" + vector.at(3) +"', assignment_date = '" + vector.at(4) +"', execution_date = '" + vector.at(5) +"', sumcost_new = '" + vector.at(7) +"' WHERE id_order = '" + vector.at(0) +"'");
    });
    th.join();
...
QVector<QString> Database_sql::getAllModelCar(QString temp, bool botemp)
{
    ...
    std::thread th([&]() {
        ...
        QSqlQuery selectQuery(db);
        if (botemp)
            selectQuery.exec("SELECT * FROM Cars WHERE id_Supplier = '" + getSupplierId(temp)
                             + "'ORDER BY Id_car ASC");
        else
...
void Database_sql::drivers_createNewData(QVector<QString> vec)
{
    ...
    std::thread th([&]() {
        QSqlQuery selectQuery(db);
        selectQuery.exec("INSERT INTO Drivers (LastName, FirstName, DadName, DriverLicense, Phone, "
                         "Email, CarNumer,Image) VALUES ('"
                         + vec[1] + "', '" + vec[2] + "', '" + vec[3] + "', '" + vec[4] + "','"
...
void Database_sql::drivers_addNewData(QVector<QString> vector)
{
    std::thread th([&]() {
        QSqlQuery selectQuery(db);
        selectQuery.exec("UPDATE Drivers SET LastName = '" + vector.at(1) + "', FirstName = '"
                         + vector.at(2) + "', DadName = '" + vector.at(3) + "',DriverLicense = '"
                         + vector.at(4) + "',Phone = '" + vector.at(5) + "',Email = '"
                         + vector.at(6) + "',CarNumer = '" + vector.at(7) + "',Image = '"
                         + vector.at(8) + "' WHERE id = '" + vector.at(0) + "'");
    });
    th.join();
}
...
void Database_sql::clients_createNewData(QVector<QString> vec)
{
    ...
    std::thread th([&]() {
        QSqlQuery selectQuery(db);
        selectQuery.exec("INSERT INTO Clients (Company, Person, Phone, EMail) VALUES ('" + vec[1]
                         + "', '" + vec[2] + "', '" + vec[3] + "', '" + vec[4] + "');");
    });
    th.join();
}
...
void Database_sql::clients_addNewData(QVector<QString> vector)
{
    std::thread th([&]() {
        QSqlQuery selectQuery(db);
        selectQuery.exec("UPDATE Clients SET Company = '" + vector.at(1) + "', Person = '"
                         + vector.at(2) + "', Phone = '" + vector.at(3) + "',EMail = '"
                         + vector.at(4) + "' WHERE Id_client = '" + vector.at(0) + "'");
    });
    th.join();
}
...
void Database_sql::cars_createNewData(QVector<QString> vec)
{
    ...
    std::thread th([&]() {
        QSqlQuery selectQuery(db);
        selectQuery.exec("INSERT INTO Cars (Id_supplier, Brand, Model, Engine_volume, Power_motor, "
                         "Body, Color, Status, Price, Image) VALUES ("
                         + vec[1] + ", '" + vec[2] + "', '" + vec[3] + "', " + vec[4] + ", "
                         + vec[5] + ", '" + vec[6] + "', '" + vec[7] + "', '" + vec[8] + "', "
                         + vec[9] + ", '" + vec[10] + "');");
    });
...
void Database_sql::cars_addNewData(QVector<QString> vector)
{
    std::thread th([&]() {
        QSqlQuery selectQuery(db);
        selectQuery.exec("UPDATE Cars SET Id_supplier = '" + vector.at(1) + "', Brand = '"
                         + vector.at(2) + "', Model = '" + vector.at(3) + "',Engine_volume = '"
                         + vector.at(4) + "', Power_motor = '" + vector.at(5) + "', Body = '"
                         + vector.at(6) + "', Color = '" + vector.at(7) + "', Status = '"
                         + vector.at(8) + "', Price = '" + vector.at(9) + "', Image = '"
                         + vector.at(10) + "' WHERE Id_car = '" + vector.at(0) + "'");
    });
    th.join();
}
...
QString Database_sql::getSupplierId(QString company)
{
    ...
    std::thread th([&]() {
        QSqlQuery selectQuery(db);
        selectQuery.exec("SELECT * FROM Supplier WHERE Company = '" + company
                         + "' ORDER BY id_supplier ASC");
...
void Database_sql::supplier_createNewData(QVector<QString> vector)
{
    std::thread th([&]() {
        QSqlQuery selectQuery(db);
        selectQuery.exec(
            "INSERT INTO Supplier (Company, Address, Representative, Phone, Email,Image) VALUES ('"
...
void Database_sql::supplier_addNewData(QVector<QString> vector)
{
    std::thread th([&]() {
        QSqlQuery selectQuery(db);
        selectQuery.exec("UPDATE Supplier SET Company = '" + vector.at(1) + "', Address = '"
                         + vector.at(2) + "', Representative = '" + vector.at(3) + "',Phone = '"
                         + vector.at(4) + "', Email = '" + vector.at(5) + "', Image = '"
                         + vector.at(6) + "' WHERE Id_Supplier = '" + vector.at(0) + "'");
    });
    th.join();
}
...
void Database_sql::delete_Data(int index, QString nameData, QString id_name)
{
    std::thread th([&]() {
        QSqlQuery selectQuery(db);
        selectQuery.exec("DELETE FROM " + nameData + " WHERE " + id_name + " = "
